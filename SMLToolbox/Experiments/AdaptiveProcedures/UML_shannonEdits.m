    ...
    methods
        ...
        function reset(uml, par)
            ...
            elseif strcmp(par.model,'gaussian')
                uml.psycfun = @(x,a,b,g,l) g+l+(1-2*l).*(1+erf((x-a)./sqrt(2*b.^2)))/2; % For function from 0 to 1
%                 uml.psycfun = @(x,a,b,g,l) g+(1-g-l).*(1+erf((x-a)./sqrt(2*b.^2)))/2;
        ...
        function update(uml, r) 
            ...
            elseif strcmp(uml.par.method, 'mean')
                ...
                % alpha
                alpha_est_tmp = sum(sum(sum(pdf_tmp.*uml.a))); % weighted average
                % beta
            ...
            if strcmp(uml.par.model,'logit')
                swpt = [logit_sweetpoints(uml.phi(end,:)) uml.par.x_lim(1) uml.par.x_lim(2)];  % note that the lambda sweet point is always at the maximum of the stimulus space
            ...
            elseif strcmp(uml.par.model,'weibull')
                swpt = [weibull_sweetpoints(uml.phi(end,:)) uml.par.x_lim(1) uml.par.x_lim(2)];  % note that the lambda sweet point is always at the maximum of the stimulus space
                swpt = max(min(swpt(uml.swpts_idx),uml.par.x_lim(2)),uml.par.x_lim(1)); % limit the sweet points to be within the stimulus space
            elseif strcmp(uml.par.model,'gaussian')
                swpt = [gaussian_sweetpoints(uml.phi(end,:)) uml.par.x_lim(1) uml.par.x_lim(2)];  % note that the lambda sweet point is always at the maximum of the stimulus space
                swpt(swpt<uml.par.x_lim(1)) = uml.par.x_lim(1); % limit the sweet points to be within the stimulus space
                swpt(swpt>uml.par.x_lim(2)) = uml.par.x_lim(2); % limit the sweet points to be within the stimulus space
% HERE                 swpt = max(min(swpt(uml.swpts_idx),uml.par.x_lim(2)),uml.par.x_lim(1)); 
            else
                error('Choose among "logit", "weibull", and "gaussian" for "uml.par.model".');
            end 
            
% HERE        %Standard n-down, 1-up algorithm
...
% HERE      
            % Just randomise intead:
            uml.current_step = rand;
            p_choose_swpt = [0.3 0.3 0.3 0.05 0.05];
            p_choose_swpt = cumsum(p_choose_swpt);
            for ii = 1:5
                uml.current_step(uml.current_step<=p_choose_swpt(ii)) = ii+1;
            end
            uml.current_step = uml.current_step - 1;
            newx = swpt(uml.current_step);
% ---
        ...
        function conf = getConf(uml, prct)
            ...
            for i = 1:length(prct)
                ...
                elseif length(uml.lambda) == 1
                    ...
                    % HERE: this is the case I'm using. Fixed the bug where
                    % the function is sometimes not monotonically
                    % increasing because of an underflow problem (the
                    % difference in values at the extremes are 0 or 1).
                    
%                     alpha_conf = interp1(cumtrapz(sum(sum(ptmp,1),3)),uml.alpha,prct(i),'linear','extrap');
%                     beta_conf = interp1(cumtrapz(sum(sum(ptmp,2),3)),uml.beta,prct(i),'linear','extrap');
                    
                    alpha_P = cumtrapz(sum(sum(ptmp,1),3));
                    alpha_x = uml.alpha;
                    alpha_x = alpha_x(alpha_P>0.0001);
                    alpha_P = alpha_P(alpha_P>0.0001);
                    alpha_x = alpha_x(alpha_P<0.9999);
                    alpha_P = alpha_P(alpha_P<0.9999);
                    alpha_conf = interp1(alpha_P,alpha_x,prct(i),'linear','extrap');
                    
                    beta_P = cumtrapz(sum(sum(ptmp,2),3));
                    beta_x = uml.beta;
                    beta_x = beta_x(beta_P>0.0001);
                    beta_P = beta_P(beta_P>0.0001);
                    beta_x = beta_x(beta_P<0.9999);
                    beta_P = beta_P(beta_P<0.9999);
                    beta_conf = interp1(beta_P,beta_x,prct(i),'linear','extrap');
...
% HERE     p = p*(1-1e-10);
    p(p==1) = 0.9999999;
    p(p==0) = 0.0000001; %% ADDED because their method doesn't remove 0's...
    p = p/sum(sum(sum(p)));
% ---
end
...
%         (psycfunc_derivative_sigma(x,phi)).^2;
    ...
    sigma2_mu = @(x) log(psycfunc(x,phi).*(1-psycfunc(x,phi))) - 2*psycfunc_derivative_mu(x,phi);

    sigma2_sigma = @(x) log(psycfunc(x,phi).*(1-psycfunc(x,phi))) - 2*psycfunc_derivative_sigma(x,phi);

    swpt_mu = fminsearch(sigma2_mu, phi(1));
% HERE
    swpt_sigma_L = fminsearch(sigma2_sigma, phi(1)-2);
    swpt_sigma_H = fminsearch(sigma2_sigma, phi(1)+2);
...
% ---
    ...
    function dpdm = psycfunc_derivative_mu(x,phi)
        ...
        gamma = phi(3);
        lambda = phi(4);
        dpdm = log( -(1-gamma-lambda)./(sqrt(2*pi)*sigma).*exp(-(x-mu).^2./(2*sigma^2)) );
    end
    function dpds = psycfunc_derivative_sigma(x,phi)
        ...
        gamma = phi(3);
        lambda = phi(4);
        dpds = log( -(1-gamma-lambda).*(x-mu)./(sqrt(2*pi)*sigma.^2).*exp(-(x-mu).^2./(2*sigma^2)) );
    end
