    ...
    for j = 1:lenCorr
        for m = 1:lenCent
            for l = 1:lenN
                ...
                % grab localiation responses for A/V and store them into corresponding matrices
                if l == 1 || l == 2; numReps = 40; else; numReps = 32; end
                for k = 1:numReps
                ...
                % calculate mean for each disc x corr, centroid
                nonEmptyA = []; nonEmptyV = [];
                for k = 1:numReps
                ...
                meanLocA_bi{i,j,m,l} = mean(nonEmptyA);
                meanLocV_bi{i,j,m,l} = mean(nonEmptyV);
            end
        end
    end
end
    ...
    for j = 1:lenCorr
        for m = 1:lenCent
            for l = 1:lenN
                ...
                % centroid location for each subject
                if ~isempty(meanLocA_bi{i,j,m,l}) 
                    VE_audCentroid_ind{i,j,m,l} = meanLocA_bi{i,j,m,l} - meanLocA_uni(l,m); end
                if ~isempty(meanLocV_bi{i,j,m,l})
                    VE_visCentroid_ind{i,j,m,l} = meanLocV_bi{i,j,m,l} - meanLocV_uni(l,m); end
            end
            % VE for each disc, corr, centroid, averaged across subjects
        ...
        % VE for each disc, corr, averaged across 6 centroids
